...
\begin{algorithm}[!ht]
\begin{algorithmic}[1]
    	\State{$NP \leftarrow User \quad assigned$}
    	\State{$ \mathbf{k}_{max} \leftarrow User \quad assigned $}
	\For{$i = 1 \rightarrow M$}
        	\State{$ \mathbf{\eta }_k \leftarrow random \quad assigned $}
        	\State{$ \mathbf{\alpha }_k \leftarrow random \quad assigned $}
        	\State{$ \mathbf{\beta }_k \leftarrow random \quad assigned $}
