...
We did this as we think that the searchspace for the rotational movement should be related to the velocity, and not to the current location. Otherwise we would be placing the random local search points on a line through the origin, which is arbitrary. \\ %TODO
...
%In the execution of our experiments we found that there were some mistakes that resulted in the generation of NaN values as positions for our individuals. Because of this the data can not be uploaded in the analyzer.\\
%The NaN values arise since the velocities, and therefore positions, and therefore function evaluation values are exploding to infinity. This might have something to do with the fact that the partial derivative is calculated in first order approximation. The derivative in a point is calculated by taking the difference of the values of two points close by the position of the shark and dividing by the distance between the two points. This can be difficult when the range or value of one variable is significantly different from another variable. We might be because these extreme values could more easily cause rounding errors.
...
% What we expected for results was an ECDF which shows for a certain amount of evaluations the best value reached and on the y-axis the amount of runs that have reached this value. The x-axis is inverted, so it begins on the left with the lowest evaluation value that every run has reached. The better the value, the fewer runs reached the value. This means that the line slowly descends to the point that no run has reached a value. The speed with which the line descends to 0 shows the quality of the algorithm in one glance. For this reason we wanted to compare it to another well known algorithm and plot them together to see how well our algorithm performs. This algorithm is called BIPOP-CMA-ES and the ECDF plot of this algorithm for a budget of 200 is shown in figure \ref{fig:BIPOP}
% \begin{figure}[h!]
%     \centering
%     \includegraphics[scale=0.6]{newplot.png}
%     \caption{ECDF plot of BIPOP-CMA-ES}
%     \label{fig:BIPOP}
% \end{figure}{}
